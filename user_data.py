    ...
    if secrets[0] is not None and secrets[1] is not None:
        ...
        if temp_deviation > 2 or temp_deviation < -2:
            ...
            for index, element in enumerate(outfit_tried):
                ...
                if (
                    element_significance > 0.2 + temp_coefficients[index]
                    or element_significance < temp_coefficients[index] - 0.2
                ):
                    return False
        return True
    else:
        user = input(
            f"Did your outfit {outfit_tried} = {translate_outfit(my_closet,outfit_tried)} feel appropriate in temperature of {temp_global} (t or f)? "
        ).lower()
        if user == "f":
            return False
        elif user == "t":
            ...
            raise ValueError("Expected either t or f but got something else!")
...
def generate_data(data_amount, *secrets):
    ...
    if not secrets:
        ...
        temp_coefficients = [
            sum(temp_coefficients[i]) / (len(temp_coefficients) - 1)
            for i in range(0, 4)
        ]
        print(
...
def main():
    ...
    if secrets == "t":
        ...
        while weather:
            ...
            suggested_outift = [
                int(element)
                for element in suggest_outfit(weather_input, outfit_output, weather)
            ]
            print(
